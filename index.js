...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Eae @${num.split('@')[0]}\nBem Vindo ao nossa *${mdata.subject}*`
				...
				teks = `Adeus e...não fará falta @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Aguarde um pouco...ZeroTwoÉaMelhor ⌛',
				success: '✔️ Completo ✔️',
				error: {
				...
				only: {
					...
					admin: '❌ Este comando só pode ser usado por administradores de grupo!Ou pelo ban Meu criador ❌',
			...
			switch(command) {
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Removi esse merda,que cara inconveniente : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Sukes menonak tifkan mode simi di group ini ✔️')
					} else {
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Já tá ativo mano')
						welkom.push(from)
						...
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('a opção de boas vindas foi desativada✔️️')
					} else {
